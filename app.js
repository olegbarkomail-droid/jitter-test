...
const DEFAULT_CONFIG = {
  ...
  STICK_THRESHOLD:     0.02, // Порог палочки (0–1). Удары выше порога = удар. Снижено: метроном не в микрофон.
...
function analyzeAudio(audioBuffer, config, gridOffsetMs, bpm) {
  ...
  while (beatMs - config.SEARCH_WINDOW_MS < durationMs) {
    ...
    if (winEndMs >= 0 && winStartMs <= durationMs) {
      ...
      // Ищем самый сильный трансиент в окне.
      // Внутри поискового окна используем порог × 0.7 — это позволяет поймать
      // чуть более тихие удары, которые гарантированно должны быть в этом месте.
      // Порог для входа в трансиент: 70 % от STICK_THRESHOLD.
      // Порог для выхода из трансиента (хвост затухания): 50 % от STICK_THRESHOLD.
      const searchThresh = config.STICK_THRESHOLD * 0.7;
      const exitThresh   = config.STICK_THRESHOLD * 0.5;
      let bestAmp = -1;
      let bestIdx = -1;
      let k = winStart;

      while (k <= winEnd) {
        if (envelope[k] > searchThresh) {
          // Вошли в трансиент — ищем его пик
          let eventAmp = envelope[k];
          let eventIdx = k;
          let j = k + 1;

          while (j <= winEnd && envelope[j] > exitThresh) {
...
function startMetronome() {
  ...
  // Android переводит AudioContext в 'suspended'. Вешаем обработчик один раз —
  // он автоматически возобновляет воспроизведение после переключения BT.
  if (!ctx._btHandlerAttached) {
    ctx.onstatechange = () => {
      if (ctx.state === 'suspended') ctx.resume().catch(() => {});
    };
    ctx._btHandlerAttached = true;
  }
...
function syncCalibHandles() {
  const stick = clamp(state.config.STICK_THRESHOLD, 0.003, 0.50);
  const stickPct = (stick / METER_MAX) * 100;

  const hs = el('calib-handle-stick');
  if (hs) hs.style.bottom = stickPct + '%';

  setText('calib-tag-stick',    stick.toFixed(3));
  setText('calib-legend-stick', stick.toFixed(3));
  setText('calib-legend-noise', stick.toFixed(3));
...
function applyThreshold(stick) {
  stick = clamp(stick, 0.003, 0.50);
  state.config.STICK_THRESHOLD = Math.round(stick * 1000) / 1000;
  storage.set(SK.CONFIG, state.config);

  // Синхронизируем слайдер «Параметры анализа»
  const ss = el('cfg-stick-thresh');
  if (ss) { ss.value = state.config.STICK_THRESHOLD; setText('cfg-stick-thresh-val', state.config.STICK_THRESHOLD.toFixed(3)); }
...
function calibLoop() {
  ...
  if (calib.auto) {
    ...
    } else if (calib.floorSamples.length && calib.noiseFloor === 0.001) {
      // Фиксируем шумовой пол один раз: медиана стартовых кадров
      const s = calib.floorSamples.slice().sort((a, b) => a - b);
      calib.noiseFloor = Math.max(0.001, s[Math.floor(s.length / 2)]);
    }

    const evThresh = Math.max(0.003, calib.noiseFloor * 3);
    if (!calib.inEvent) {
...
async function startCalibration() {
  ...
  try {
    ...
    // Метроном пользователь запускает вручную через кнопку ▶.
    // Здесь его НЕ запускаем — в микрофон он не попадает, аудио-сессию не нарушает.
...
async function startRecording() {
  ...
  try {
    ...
    // Фиксируем смещение первого удара метронома от начала записи (мс).
    //
    // После каждого прохода планировщика nextNoteTime = ПЕРВЫЙ НЕзапланированный удар,
    // т.е. уже за горизонтом 100 мс. Реальный следующий слышимый удар —
    // это последний ЗАПЛАНИРОВАННЫЙ = nextNoteTime − beatInterval.
    //
    // Алгоритм:
    //   rawMs    = nextNoteTime − currentTime   (100..100+beatInterval мс)
    //   offsetMs = rawMs − beatInterval          (время до последнего запланированного)
    //   Если offsetMs < 50 мс (удар слишком близко к старту) → берём следующий (+beatInterval)
    //   Итоговый offsetMs ограничен диапазоном [50, beatInterval)
    {
      const beatIntervalMs = 60000 / (state.config.BPM || 60);
      if (metronome.ctx && metronome.nextNoteTime > metronome.ctx.currentTime) {
        const rawMs    = Math.round((metronome.nextNoteTime - metronome.ctx.currentTime) * 1000);
        let   offsetMs = rawMs - beatIntervalMs;   // время до последнего запланированного удара
        if (offsetMs < 50) offsetMs += beatIntervalMs; // слишком близко → берём следующий
        state.gridOffsetMs = Math.min(offsetMs, beatIntervalMs - 50);
      } else {
        state.gridOffsetMs = 100;                  // запасное значение
      }
    }
...
async function processRecording() {
  ...
  try {
    ...
    // ── Авто-коррекция систематического сдвига ─────────────────────────────
    // Если среднее со знаком |meanSigned| > 40 мс — это явный систематический
    // сдвиг (скорее всего из-за неточного gridOffsetMs или задержки оборудования).
    // Вычитаем его из всех jitter, чтобы получить «чистый» джиттер.
    // Исходное смещение сохраняем в currentSession для отображения пользователю.
    let systematicOffsetMs = 0;
    if (stats && Math.abs(stats.meanSigned) > 40) {
      systematicOffsetMs = Math.round(stats.meanSigned);
      results.forEach(r => { r.jitterMs = Math.round(r.jitterMs - systematicOffsetMs); });
      stats = computeStats(results);  // пересчитываем с исправленными значениями
    }
    ...
    state.currentSession = {
      ...
      config:    { ...state.config },
      gridOffsetMs: state.gridOffsetMs,    // для отладки
      systematicOffsetMs,                  // сколько скорректировано
      results,
      stats,
    };
...
function renderResults(session) {
  renderStats(session.stats, session.systematicOffsetMs || 0, session.gridOffsetMs);
  renderChart(session.results);
  renderComparison();
}

/* ── Статистика ─────────────────────────────────────────── */
function renderStats(stats, systematicOffsetMs = 0, gridOffsetMs = null) {
  if (!stats) return;
  const lvl = stats.level;

  // Карточка коррекции — показываем только если сдвиг был скорректирован
  const corrCard = systematicOffsetMs !== 0
    ? `<div class="stat-card" style="border-color:#f97316">
        <div class="stat-label">⚙️ Коррекция сдвига</div>
        <div class="stat-value" style="color:#f97316">${systematicOffsetMs > 0 ? '+' : ''}${systematicOffsetMs} мс</div>
        <div class="stat-sub">Авто-скорректировано (джиттер пересчитан)</div>
       </div>`
    : '';

  // Отладочная карточка gridOffset (полезна при анализе ошибок)
  const gridCard = gridOffsetMs != null
    ? `<div class="stat-card">
        <div class="stat-label">🎯 Grid-offset</div>
        <div class="stat-value">${gridOffsetMs} мс</div>
        <div class="stat-sub">Задержка первого удара в записи</div>
       </div>`
    : '';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── BT-fix: реакция на подключение/отключение наушников ─────────
  // При смене аудиоустройства Android может перевести AudioContext
  // в suspended или полностью закрыть. Принудительно возобновляем.
