...
async function startCalibration() {
  ...
  try {
    ...
    // Новый AudioContext (calib.ctx) может «усыпить» metronome.ctx на iOS Safari.
    // Явно resume-им его, не запуская сам метроном — пользователь включает его вручную.
    ensureAudioCtx();
...
function stopCalibration() {
  ...
  // Метроном оставляем в том состоянии, в каком он был — пользователь управляет им вручную.
  // После закрытия calib.ctx resume-им metronome.ctx на случай если браузер его «усыпил».
  if (metronome.ctx && metronome.ctx.state === 'suspended') metronome.ctx.resume();
