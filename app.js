...
const calib = {
  active:      false,
  stream:      null,
  ctx:         null,   // НЕ отдельный контекст — ссылка на metronome.ctx
  src:         null,   // MediaStreamSourceNode — для disconnect()
  analyser:    null,
  buf:         null,
  raf:         null,
  smoothWin:   1,
...
async function startCalibration() {
  ...
  // ВАЖНО: ensureAudioCtx() вызывается СИНХРОННО, пока браузер ещё «помнит» жест.
  // Второй AudioContext НЕ создаём — iOS не может работать с двумя одновременно.
  const sharedCtx = ensureAudioCtx();
  ...
  try {
    ...
    // Используем тот же контекст, что и метроном — никакого нового AudioContext.
    calib.ctx = sharedCtx;

    calib.smoothWin = Math.max(1, Math.round(calib.ctx.sampleRate * 0.002));
    const src = calib.ctx.createMediaStreamSource(calib.stream);
    calib.src = src; // сохраняем для disconnect() в stopCalibration
...
function stopCalibration() {
  ...
  // calib.ctx — это та же ссылка, что metronome.ctx. НЕ закрываем!
  // Просто отключаем узлы анализатора, чтобы не засорять граф.
  if (calib.src)     { try { calib.src.disconnect(); } catch {} }
  if (calib.analyser) { try { calib.analyser.disconnect(); } catch {} }
  calib.stream = calib.ctx = calib.src = calib.analyser = calib.buf = null;
  // metronome.ctx не трогаем — метроном продолжает работать как ни в чём не бывало.
