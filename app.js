...
async function startCalibration() {
  ...
  try {
    ...
    // Получаем микрофон первым делом.
    calib.stream = await navigator.mediaDevices.getUserMedia(constraints);

    // На Android Chrome два AudioContext одновременно убивают аудио-сессию.
    // Решение: НЕ создаём новый контекст — переиспользуем metronome.ctx.
    // Если метроном ещё не запускался — создаём контекст здесь, один на всё приложение.
    const AC = window.AudioContext || window.webkitAudioContext;
    if (!metronome.ctx) metronome.ctx = new AC();
    if (metronome.ctx.state === 'suspended') await metronome.ctx.resume().catch(() => {});
    calib.ctx = metronome.ctx; // ссылка, не копия — один контекст на всё
    ...
    src.connect(calib.analyser); // анализатор НЕ подключён к destination — нет петли
...
function stopCalibration() {
  ...
  // calib.ctx — это ссылка на metronome.ctx, НЕЛЬЗЯ закрывать!
  // Просто отключаем анализатор от графа и обнуляем ссылки.
  if (calib.analyser) { try { calib.analyser.disconnect(); } catch (_) {} }
