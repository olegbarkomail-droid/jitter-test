...
function stopCalibration() {
  ...
  // iOS Safari может «усыпить» metronome.ctx пока работал calib.ctx.
  // Надёжнее всего — закрыть и обнулить его: следующее нажатие ▶ Метроном
  // создаст свежий AudioContext прямо из click-жеста, что iOS всегда разрешает.
  if (metronome.ctx) {
    if (metronome.isPlaying) stopMetronome();
    metronome.ctx.close().catch(() => {});
    metronome.ctx = null;
  }
